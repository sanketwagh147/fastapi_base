    ...
    for py_file in routes_dir.rglob("*.py"):
        ...
        try:
            ...
            if not isinstance(router, APIRouter):
                raise RouterDiscoveryError(
                    f"Router file '{py_file.name}' exports 'router' but it's not an APIRouter instance.\n"
                    f"  File: {py_file}\n"
                    f"  Module: {module_path}\n"
                    f"  Type: {type(router).__name__}\n"
                    f"  Expected: APIRouter instance"
                )
            ...
            if hasattr(module, "ROUTER_CONFIG"):
                router_config = getattr(module, "ROUTER_CONFIG")
        ...
        except RouterDiscoveryError:
            # Re-raise our custom errors as-is
            raise
        except ImportError as e:
            raise RouterDiscoveryError(
                f"Failed to import router file '{py_file.name}'.\n"
                f"  File: {py_file}\n"
                f"  Module: {module_path}\n"
                f"  Error: {type(e).__name__}: {e}\n"
                f"  Hint: Check for syntax errors or missing dependencies"
            ) from e
        except Exception as e:
            raise RouterDiscoveryError(
                f"Unexpected error loading router from '{py_file.name}'.\n"
                f"  File: {py_file}\n"
                f"  Module: {module_path}\n"
                f"  Error: {type(e).__name__}: {e}"
            ) from e
...
def register_routers(app: FastAPI, routes_dir: Path | None = None) -> None:
    ...
    except RouterDiscoveryError as e:
        print(f"\n❌ ROUTER DISCOVERY FAILED\n")
        print(f"{e}\n")
        print("=" * 70)
        print("App startup aborted due to router discovery error.")
        print("Files starting with _ are ignored (e.g., _example.py).")
        print("=" * 70)
        raise
    ...
    for router, config in routers:
        try:
            ...
            raise RouterDiscoveryError(
                ...
                f"  Config: {config}\n"
                f"  Error: {type(e).__name__}: {e}\n"
                f"  Hint: Check for duplicate prefixes or invalid configuration"
            ) from e
    
    print(f"\n✅ Successfully registered {len(routers)} router(s)")
